...
{    
	float yfmod4 = floor( fmod( y, 4.0 ) );    
    float xfmod8 = floor( fmod( x, 8.0 ) );
    float yfmod8 = floor( fmod( y, 8.0 ) );
    
    // dark orange
    float idx = 2.0;
   
    // black
    idx = yfmod4 == 0.0 ? 1.0 : idx;
    idx = xfmod8 == ( yfmod8 < 4.0 ? 3.0 : 7.0 ) ? 1.0 : idx;
...
void DrawCastle( thread float3& color, float x, float y )
{
	...
	{
		float yfmod4    = fmod( y, 4.0 );
		float xfmod8    = fmod( x, 8.0 );
		float xfmod16_4 = fmod( x + 4.0, 16.0 );
		float xfmod16_3 = fmod( x + 5.0, 16.0 );
		float yfmod8    = fmod( y, 8.0 );

		// dark orange
		float idx = 2.0;

		// black
		idx = yfmod4 == 0.0 && y <= 72.0 && ( y != 44.0 || xfmod16_3 > 8.0 ) ? 1.0 : idx;
		idx = x >= 24.0 && x <= 32.0 && y >= 48.0 && y <= 64.0 ? 1.0 : idx;
		idx = x >= 48.0 && x <= 56.0 && y >= 48.0 && y <= 64.0 ? 1.0 : idx;
		idx = x >= 32.0 && x <= 47.0 && y <= 25.0 ? 1.0 : idx;
		idx = xfmod8 == ( yfmod8 < 4.0 ? 3.0 : 7.0 ) && y <= 72.0 && ( xfmod16_3 > 8.0 || y <= 40.0 || y >= 48.0 ) ? 1.0 : idx;  

		// white
		idx = y == ( xfmod16_4 < 8.0 ? 47.0 : 40.0 ) ? 3.0 : idx;
		idx = y == ( xfmod16_4 < 8.0 ? 79.0 : 72.0 ) ? 3.0 : idx;
		idx = xfmod8 == 3.0 && y >= 40.0 && y <= 47.0 ? 3.0 : idx;
		idx = xfmod8 == 3.0 && y >= 72.0 ? 3.0 : idx;

		// transparent
		idx = ( x < 16.0 || x >= 64.0 ) && y >= 48.0 ? 0.0 : idx;
		idx = x >= 4.0  && x <= 10.0 && y >= 41.0 && y <= 47.0 ? 0.0 : idx;
		idx = x >= 68.0 && x <= 74.0 && y >= 41.0 && y <= 47.0 ? 0.0 : idx;             
		idx = y >= 73.0 && xfmod16_3 > 8.0 ? 0.0 : idx;
...
{
	return worldY - 4.0 * 16.0 - floor( 64.0 * ( 1.0 - fabs( 2.0 * ( clamp( ( time - coinTime ) / 0.8, 0.0, 1.0 ) ) - 1.0 ) ) );
}

float QuestionAnimY( float worldY, float time, float questionHitTime )
{
     return worldY - 4.0 * 16.0 - floor( 8.0 * ( 1.0 - fabs( 2.0 * clamp( ( time - questionHitTime ) / 0.25, 0.0, 1.0 ) - 1.0 ) ) );
}
...
void DrawGame( thread float3& color, float time, float pixelX, float pixelY, float screenWidth, float screenHeight )
{
    ...
    {
        goombaLifeTime 	= 10.3;
		goombaX 		= worldX - 790.0 - floor( fabs( fmod( ( min( time, goombaLifeTime ) + 6.3 ) * GOOMBA_SPEED, 2.0 * 108.0 ) - 108.0 ) );
    }
    ...
    if ( goombaX < 0.0 ) goombaX = worldX - 766.0 - floor( fabs( fmod( ( time + 6.3 ) * GOOMBA_SPEED, 2.0 * 108.0 ) - 108.0 ) );
    if ( goombaX < 0.0 ) goombaX = worldX - 638.0 - floor( fabs( fmod( ( time + 6.6 ) * GOOMBA_SPEED, 2.0 * 84.0 ) - 84.0 ) );
